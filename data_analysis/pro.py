...
class MyApp(ctk.CTk):
    def __init__(self):
        ...
        self.start_button.grid(row=14, column=1, columnspan=2, padx=10, pady=50, sticky="nsew")
    ...
    def plot_df(self, df, item, folder_name="plots", min_limit=0.4, max_limit=0.6, previous_plots_list=[]):
        if len(df) < 3:
        ...
        if label:
            ...
            return child_folder_2, final_optimum, fig_name
        return None, None, None
    ...
    def start(self):
        ...
        if self.custom_limits_var.get():
            ...
            min_limit_list = [min_limit]
            max_limit_list = [max_limit]
        else:
        ...
        for i in range(len(min_limit_list)):
            min_limit = min_limit_list[i]
            max_limit = max_limit_list[i]
            start = len(df) + 1
            ...
            for item in sequences:
                ...
                if len(temp_df) > max_points:
                    continue
                child_folder, final_difference, fig_name = self.plot_df(temp_df, item, folder_name, min_limit, max_limit, previous_plots)
